...
var Link = function(obj) 
{
	...
	this.value = obj.value;		//Plus général que this.emd. L'utilisateur défini quel est la valeur du lien en fonction de la signature choisie et de la méthode de calcul des liens (euclidiennes, etc...)
};
...
var Graph = function(data, config) 
{
	...
	var features_gui = this.datgui.addFolder('Signatures');
	...
	var allFeatures = function()
	{
		this.Shape_Roundness = function()
		{
			...
			console.log("Signature selectionnee : " + config.chosenFeature1);
		}
	}
...
//Calcul un nombre finis de liens utilisant la méthode de la distance euclidienne pour connaitre la longueur du lien entre deux noeuds.
//La signature this.graph.chosenFeature1 est prise en compte.
Graph.prototype.computeLinks_UsingEuclideanDistance = function(nodes)
{
	...
	for(var i = 0 ; i < nodes.length - 1 ; i++)
	{
		...
		for(var j = 0 ; j < nodes.length -1; j++)
		{
			...
			if(i != j)	//Ne pas prendre une signature comparée à elle même
			{
				if(euclideanDistance < min)
				{
					...
					//On garde en mémoire un nombre fini de minimums pour créer plusieurs liens
		...
		//Réinitialisation des variables pour le prochain noeud
		min = 5000;
		minPos = -1;
...
//**************************************************************
Graph.prototype.computeMeanAndSTD = function(nodes)
{
	var f = this.config.chosenFeature1;
	console.log("Calcul de la moyenne et de l'ecart type pour la signature : " + f + "...");
	
	var mean = 0;
	var std = 0;
	
	//-1 car le dernier noeud contient des valeurs non définis. Pourquoi ?
	for(i = 0 ; i < nodes.length - 1 ; i++)
	{
		mean += +nodes[i].features[f];			// !!!nodes[i].features[f] est un chaine de caractères !
	}
	mean /= nodes.length - 1;
	...
	console.log("moyenne : ");
	console.log(this.config.chosenFeature1_Mean);
	console.log("ecart type :");
	console.log(this.config.chosenFeature1_std);
}

//Standardised normal distribution (variable centrée réduite)
Graph.prototype.computeSND = function(nodes)
{
	var f = this.config.chosenFeature1;
	var m = this.config.chosenFeature1_Mean;
	var std = this.config.chosenFeature1_std;
	// = V - mean / std
...
Graph.prototype.update = function()
{
	...
	this.computeMeanAndSTD(nodes);
	this.computeSND(nodes);
	
	console.log("Fin du calcul des valeurs centrees reduites pour la signature : " + this.config.chosenFeature1 + ".");
	console.log("---");
	
	if(this.config.linkingMethod == "Euclidean Distance")
	{
		this.computeLinks_UsingEuclideanDistance(nodes);
